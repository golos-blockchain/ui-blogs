...
export default async function getState(api, url, options, offchain = {}) {
    ...
    if (parts[0][0] === '@') {
       ...
       for (let key in replies) {
            ...
            const donates = await api.getDonates({author: reply.account, permlink: reply.permlink}, '', '', 10, 0);
            state.content[link].donate_list = donates;  
        }

        const donates = await api.getDonates({author: account, permlink: permlink}, '', '', 10, 0);
